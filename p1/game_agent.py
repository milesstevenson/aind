...
class CustomPlayer:
    ...
    def alphabeta(self, game, depth, alpha=float("-inf"), beta=float("inf"), maximizing_player=True):
        ...
        if maximizing_player:
            best_value = alpha
            for move in moves:
                score, _ = self.alphabeta(game.forecast_move(move), depth - 1, best_value, beta, not maximizing_player)
                if score > best_value:
                    best_value = score
                    best_move = move
                if best_value >= beta:
            ...
            best_value = beta
            for move in moves:
                score, _ = self.alphabeta(game.forecast_move(move), depth - 1, alpha, best_value, not maximizing_player)
                if score < best_value:
                    best_value = score
                    best_move = move
                if best_value <= alpha:
